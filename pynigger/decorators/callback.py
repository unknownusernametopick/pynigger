...
class Callback(OnCallbackQuery):
    ...
    def callback(
        ...
        Examples:
            ...
            @Nigger.callback(['first_button', 'second_button'])
            ...
            @Nigger.callback('first_button', owner_only=True)
            ...
            # Others will be ignored.
            @Nigger.callback('first_button', sudo_only=True)
            ...
            @Nigger.callback(filters=filters.user(['GerProgrammer', 'Designatory']))

            # Filter only queries done in 'NiGGeR_BotsChat'
            @Nigger.callback(filters=filters.chat('NiGGeR_BotsChat'))

            # Filter only queries ending with the word 'baby'.
            @Nigger.callback(filters=filters.regex(r'baby$'))

            # Filter all queries with the word 'hello' AND which are done in 'NiGGeR_BotsChat'.
            @Nigger.callback(filters=filters.chat('NiGGeR_BotsChat') & filters.regex('hello'))
            # or
            @Nigger.callback('hello', filters=filters.chat('NiGGeR_BotsChat'))
            ...
            @Nigger.callback(filters=~filters.chat('NiGGeR_BotsChat') & filters.regex('hello'))
            # or
            @Nigger.callback(filters=filters.regex('hello') & ~filters.chat('NiGGeR_BotsChat'))
            ```
        """
        # ToDo:
