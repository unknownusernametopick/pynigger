...
class Command(OnMessage):
    ...
    def command(
        ...
        Parameters:
            ...
            extra_filters (pyrogram.filters, optional): Extra filters to apply in your function. Import ``filters`` from pyrogram or pynigger to use this. See example below.
        ...
        Examples:
            ...
            @Nigger.command('greet', description='Greet the user')
            ...
            @Nigger.command(extra_filters=filters.media)

            # Filter only text messages.
            @Nigger.command(extra_filters=filters.text)

            # Filter only messages sent by 'GerProgrammer'.
            @Nigger.command(extra_filters=filters.user('GerProgrammer'))

            # Filter only messages sent in 'NiGGeR_BotsChat'
            @Nigger.command(extra_filters=filters.chat('NiGGeR_BotsChat'))

            # Filter only messages with the word 'baby'.
            @Nigger.command(extra_filters=filters.regex('baby'))

            # Filter all media messages sent by bots.
            @Nigger.command(extra_filters=filters.bot & filters.media)

            # Filter all messages sent by bots except media messages.
            @Nigger.command(extra_filters=filters.bot & ~filters.media)

            # Filter either media messages or text messages.
            @Nigger.command(extra_filters=filters.text | filters.media)
            ```
        """
