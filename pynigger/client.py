...
from pynigger.decorators import Mechanism
from pynigger.config import ENV, settings
from pynigger.constants import __version__
from inspect import getmembers, isfunction
from pynigger.plugins.models.bans import Bans
...
class Nigger(Client, Mechanism):
    __support__ = "NiGGeR_BotsChat"
    __updates__ = "pynigger"
    ...
    def activate(self):
        """Main method of `Nigger` class which loads plugins and activates/runs your bot."""
        module = settings()
        try:
            ...
            if plugins:
                if isinstance(plugins, list):
                    for folder in plugins:
                        Nigger.log(f"Searching for plugins in '{folder}'...")
            ...
            if addons:
                ...
                for i in addons:
                    path = f"pynigger/plugins/addons/{i}"
    ...
    def data(self) -> dict:
        ...
        | **all_commands**   | dictionary of command and their descriptions if passed in ``Nigger.cmd`` decorator else None |
    ...
    def all_plugins(self) -> dict:  # not "plugins" to prevent clash with pyrogram's "plugins" attribute
        ...
        ```python
        {
            ...
            "must_join":"C:\\Users\\...\\pynigger\\addons\\must_join.py"
        },
        ```
        """
