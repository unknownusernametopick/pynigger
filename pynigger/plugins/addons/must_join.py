...
@Nigger.cmd(private_only=True,
            extra_filters=~filters.edited & filters.incoming,
            group=-1)
async def must_join_channel(bot: Nigger, msg: Message):
    if ENV.DATABASE_URL:
        from pynigger.plugins.helpers import db

        if (await db.has_table("bans")) and msg.from_user.id in (
                await db.get_all_primary_keys("bans")):
    ...
    if not chats:
        Nigger.log(
            "Addon must_join is loaded but no chat is listed in MUST_JOIN var of settings",
            "warn",
        )
        return
    nope = []
    for chat in chats:
        try:
            ...
            Nigger.log(f"I'm not admin in the {chat} Chat !", "warning")
    if not nope:
        return
    buttons = []
    for i in nope:
        ...
        if isinstance(i, int):
            i = c.title
        buttons.append(
            [InlineKeyboardButton(f"✨ Join {i} ✨", url=c.invite_link)])
    try:
        await msg.reply(
