...
@Nigger.cmd(private_only=True, extra_filters=~filters.edited & filters.incoming, group=-1)
async def must_join_channel(bot: Nigger, msg: Message):
    ...
    try:
        ...
        chats = []
    if not chats:
        Nigger.log("Addon must_join is loaded but no chat is listed in MUST_JOIN var of settings", "warn")
        return
    nope = []
    for chat in chats:
        try:
