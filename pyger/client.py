...
from pyger.logger import logger
from pyrogram.types import BotCommand
from pyrogram import Client, idle, raw
from pyger.decorators import Mechanism
from pyger.config import ENV, settings
from pyger.constants import __version__
from inspect import getmembers, isfunction
from pyger.plugins.models.bans import Bans
from pyger.plugins.models.users import Users
from pyger.helpers.localization import l10n_setup, get_all_langs
from pyger.decorators.command import command_data
...
class Ger(Client, Mechanism):
    ...
    def __init__(self, **kwargs):
        global __printed__
        if not __printed__:
            print(f'Welcome to Pyger [v{__version__}]')
            print('Copyright (C) 2021-2022 Ger Bots <https://github.com/unknownusernametopick> \n')
            __printed__ = True
        env = ENV()
        super().__init__(
            ":memory:",
    ...
    def activate(self):
        """Main method of `Ger` class which loads plugins and activates/runs your bot."""
        module = settings()
        try:
            ...
            if plugins:
                if isinstance(plugins, list):
                    for folder in plugins:
                        Ger.log(f"Searching for plugins in '{folder}'...")
    ...
    def list_modules(directory):
        ...
        if not os.path.exists(directory):
            Ger.log(f"No directory named '{directory}' found")
            return
        if "/." not in directory:
    ...
    def data(self) -> dict:
        ...
        | **all_commands**   | dictionary of command and their descriptions if passed in ``Ger.cmd`` decorator else None |
    ...
    def all_plugins(self) -> dict:  # not "plugins" to prevent clash with pyrogram's "plugins" attribute
        ...
        ```python
        {
            ...
            "must_join":"C:\\Users\\...\\pyger\\addons\\must_join.py"
        },
        ```
        """
